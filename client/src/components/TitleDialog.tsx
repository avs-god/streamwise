...
export function buildTitleDiscussionHref(title: { id: number; mediaType: "movie" | "tv"; title: string }) {
  return `/community?tmdbId=${title.id}&mediaType=${title.mediaType}&title=${encodeURIComponent(title.title)}`;
}
...
export default function TitleDialog({ titleId, mediaType, region, language, open, onOpenChange }: { titleId: number | null; mediaType: "movie" | "tv" | null; region: string; language: string; open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  useEffect(() => { if (!open) { setPlannedFor("this_month"); setProviderFilter("all"); setMonitorAvailability(true); } }, [open]);
  ...
  function save() {
    ...
    if (!title) return;
    addToWatchlist.mutate({ tmdbId: title.id, mediaType: title.mediaType, title: title.title, posterPath: title.posterPath, releaseDate: title.releaseDate, plannedFor, monitorAvailability, availabilityRegion: region, providerNames: title.offers.map(offer => offer.name), offers: title.offers.map(offer => ({ id: offer.id, name: offer.name, type: offer.type })), availabilityCheckedAt: new Date(title.checkedAt), availabilitySourceUrl: title.providerPageUrl });
  }
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-h-[92vh] overflow-y-auto border-[#d9d3c4] bg-[#fcfaf4] p-0 sm:max-w-2xl"><div className="p-5 sm:p-7">
    {titleQuery.isLoading ? <div className="space-y-4"><Skeleton className="h-8 w-2/3" /><Skeleton className="h-20 w-full" /><Skeleton className="h-24 w-full" /></div> : !title || !titleQuery.data?.configured ? <><DialogHeader><DialogTitle className="serif text-2xl text-[#1e4a3a]">Availability is not connected</DialogTitle><DialogDescription>Connect a supported catalog token to inspect current legal options for this title. Streamwise does not display guessed availability.</DialogDescription></DialogHeader></> : <>
      ...
      <div className="mt-6 rounded-xl border border-[#dfd8c9] bg-white/65 p-4"><div className="flex items-start justify-between gap-4"><div><p className="eyebrow">Legal options in {region}</p><p className="mt-1 text-sm text-[#597067]">Checked {format(new Date(title.checkedAt), "PPp")}. A provider can change an offer after this check.</p><p className="mt-1 text-xs text-[#75847c]">Availability data: JustWatch via TMDb.</p></div>{title.providerPageUrl && <a className="inline-flex shrink-0 items-center gap-1 text-sm font-semibold text-[#205241] underline underline-offset-4" href={title.providerPageUrl} target="_blank" rel="noreferrer">Open source <ExternalLink className="size-3.5" /></a>}</div>
      ...
      <div className="mt-5 rounded-xl bg-[#e8f0e9] p-4"><p className="text-sm font-semibold text-[#20483a]">Save the evidence, not a guess</p><p className="mt-1 text-xs leading-5 text-[#557167]">We store this title, your viewing intent, the selected country, and typed offer categories reported at this check. That snapshot is the only availability input used in your subscription decisions.</p><div className="mt-3 flex items-center justify-between gap-4 rounded-lg border border-[#c6d8c9] bg-white/55 px-3 py-2.5"><div><label htmlFor="monitor-title" className="text-sm font-semibold text-[#214a3a]">Track later offer changes</label><p className="mt-0.5 text-xs leading-5 text-[#5e7469]">Opt in to compare future checks with this snapshot and show observed changes in your private updates feed.</p></div><Switch id="monitor-title" checked={monitorAvailability} onCheckedChange={setMonitorAvailability} aria-label="Track later offer changes" /></div><div className="mt-3 flex flex-col gap-2 sm:flex-row"><Select value={plannedFor} onValueChange={value => setPlannedFor(value as typeof plannedFor)}><SelectTrigger className="bg-white sm:w-48"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="this_week">Watch this week</SelectItem><SelectItem value="this_month">Watch this month</SelectItem><SelectItem value="someday">Keep for later</SelectItem></SelectContent></Select><Button onClick={save} disabled={addToWatchlist.isPending} className="bg-[#1e4a3a] text-[#fbf8ee] hover:bg-[#153a2d]">{user ? <><BookmarkPlus className="size-4" />Save to watchlist</> : "Sign in to save"}</Button><Link href={buildTitleDiscussionHref(title)} onClick={() => onOpenChange(false)} className="inline-flex items-center justify-center rounded-md border border-[#9eb6a4] bg-white px-4 py-2 text-sm font-semibold text-[#27543f]">Discuss this title</Link></div></div>
    </>}
