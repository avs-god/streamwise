...
export default function CatalogOfferPreview({ titleId, mediaType, region, language }: { titleId: number; mediaType: "movie" | "tv"; region: string; language: string }) {
  ...
  const offerLimit = showAll ? offers.length : 4;
  const comparisons = [
  ...
  return <>
    <div className="streamwise-offer-preview mt-4 rounded-xl border border-[#c7d9ca] bg-[#edf5ee] p-3" aria-label={`Legal offer comparison in ${region}`}>
      ...
      {offers.length ? <><div className="mt-2 flex flex-wrap gap-1.5">{offers.slice(0, offerLimit).map(offer => <a key={`${offer.type}-${offer.id}`} href={offer.webUrl ?? undefined} target="_blank" rel="noreferrer" className="rounded-full border border-[#bad0bd] bg-white px-2 py-1 text-xs font-semibold text-[#315c49]">{offer.name} · {offerType[offer.type]}</a>)}</div>{offers.length > 4 ? <button type="button" onClick={() => setShowAll(value => !value)} className="mt-2 min-h-9 text-xs font-bold text-[#2f634f] underline underline-offset-4">{showAll ? "Show fewer offers" : `Show all ${offers.length} primary-source offers`}</button> : null}{offers.some(offer => /youtube|google tv|google play/i.test(offer.name)) ? <p className="mt-2 rounded-lg bg-white/65 px-2 py-1.5 text-[0.66rem] leading-4 text-[#46675a]">YouTube / Google TV availability is source-backed and marked as included, rental, or purchase above.</p> : null}</> : <p className="mt-1 text-xs leading-5 text-[#587268]">No primary-source offer is currently reported for this country.</p>}
      <p className="mt-2 text-[0.65rem] text-[#687d73]">JustWatch via TMDb · checked {new Date(title.checkedAt).toLocaleDateString()}</p>
      <div className="mt-3 grid gap-2">{comparisons.map(source => <div key={source.label} className="rounded-lg border border-[#d1dfd3] bg-white/70 px-2.5 py-2"><p className="text-xs font-bold text-[#426755]">{source.label}</p>{source.status === "available" && source.offers.length ? <div className="mt-1 flex flex-wrap gap-1">{source.offers.slice(0, showAll ? source.offers.length : 3).map(offer => <a key={`${source.label}-${offer.id}`} href={offer.webUrl ?? undefined} target="_blank" rel="noreferrer" className="rounded-full border border-[#ccd9ce] bg-white px-2 py-1 text-[0.68rem] font-semibold text-[#315c49]">{offer.name} · {offerType[offer.type]}{offer.detail ? ` · ${offer.detail}` : ""}{offer.price ? ` · ${offer.price}` : ""}</a>)}</div> : <p className="mt-1 text-[0.68rem] leading-4 text-[#687d73]">{source.status === "not_configured" ? "This provider source is not configured." : "No matching offer or a temporary source response is available."}</p>}<p className="mt-1 text-[0.62rem] text-[#75867e]">{source.checkedAt ? `Checked ${new Date(source.checkedAt).toLocaleDateString()}` : "No check time available"}</p></div>)}</div>
