...
## Personalization, mobile usability, recommendation chat, and multi-platform offers — 2026-08-27

Settings now persist member-selected recommendation genres, original languages, maximum runtime, film/series inclusion, preferred legal-offer country, interface density, and reduced motion. The country preference is applied to Discovery and Recommendations only when the member has not already chosen a country during the current visit. The mobile drawer now includes Settings with minimum touch-target sizing, and the reduced-motion preference suppresses non-essential interface movement after save.

Recommendation chat now has an explicit film/series scope control, clearer filter layout, additional starter prompts, a pending-response state, keyboard send guidance, and a restart-conversation control. The explicit single-type scope reaches the catalog intent; the `Both` control leaves the user’s natural-language request authoritative. Legal card previews now expose every returned first-source offer through an explicit expansion control and preserve included, ad-supported, free, rental, and purchase classifications. YouTube, YouTube Movies, Google TV, and Google Play aliases remain individual source-returned availability options and are never inferred from public-web discussion.

Validation passed **36 Vitest files / 130 tests** with **one intentionally skipped opt-in health file**, **13 Playwright checks**, TypeScript, and production build. Phone-width screenshots verified Discovery, expanded Settings, and recommendation chat without horizontal overflow. A standalone compiled server returned HTTP `200` for `/`, `/settings`, `/recommendations`, `/updates`, `/leaving-soon`, `/manifest.webmanifest`, and `/sw.js`; the temporary process was stopped afterwards. The build retains Vite’s non-blocking large-chunk advisory.

## Mobile Community navigation and thread reader — 2026-08-27

The mobile drawer now keeps **Community** and **Assistant** in the first, explicitly named Explore section and makes the remaining menu content independently scrollable within the safe viewport. This prevents lower navigation entries from being clipped by the PWA install area on short phone screens. Assistant is available as both a primary drawer link and the persistent launcher.

Community now uses a forum-oriented layout with horizontal topic navigation, an explicit observation filter, thread cards that open a focused reader, and a nested comment tree. Members can reply at any depth, return to a top-level reply, flag spoilers, and submit private reports from individual comments. Existing immediate-public community behavior, source-linked unverified labels, role-gated moderation, and legal-offer separation are unchanged.

Validation passed **37 Vitest files / 131 tests** with **one intentionally skipped opt-in health file**, **13 Playwright checks**, TypeScript, production build, and phone-width Community/Assistant visual review. The production build retains Vite’s non-blocking large-chunk advisory.
