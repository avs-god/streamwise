...
export async function researchDiscoveryLead(input: { query: string; region: string; language: string }): Promise<DiscoveryResearch> {
  ...
  if (!structured || structured.status === "insufficient" || (!sourceGroups.sources.length && !sourceGroups.communitySources.length)) return insufficient("No source-linked public-web lead met Streamwise’s evidence boundary for this question. Check the legal catalog for current country-specific offers instead.");
  return { status: "lead", summary: structured.summary, ...sourceGroups, searchedAt: new Date().toISOString(), limitation: "All public-web and community items are unverified leads, not availability, leaving-soon, or platform-transfer facts. They do not affect tracking, alerts, or subscription decisions. Confirm current offers in the legal catalog and inspect each source directly." };
}
